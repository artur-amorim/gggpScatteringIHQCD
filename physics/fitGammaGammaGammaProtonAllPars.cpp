...
int main(int argc, char ** argv)
{
    ...
    auto f = [&f2photon, &f2, &fl, &sigma_gp, &sigma_gg, &f2photon_pts, &f2_pts, &fl_pts, &sigma_gp_pts, &sigma_gg_pts, &hard] (const std::vector<double> & X)
    {
        ...
        cout << "k1: " << ks[0] << " k2: " << ks[1] << " k3: " << ks[2] << " k4: " << ks[3] << endl;
        cout << "kbar1: " << kbars[0] << " kbar2: " << kbars[1] << " kbar3: " << kbars[2] << " kbar4: " << kbars[3] << endl;
    ...
    vector<double> delta = {0.5, 0.5, 0.5, 0.5, 0.5, 0.01, 0.01, 0.1, 0.1, 10, 1, 0.1, 10};
    ...
    vector<double> ks = {X_opt[5], X_opt[6], X_opt[7], X_opt[8]};
    vector<double> kbars = {X_opt[9], X_opt[10], X_opt[11], X_opt[12]};
    vector<double> Im_gn_gg(reggeons.size()), Im_gn_gp(reggeons.size());
    for(int i = 0; i < reggeons.size(); i++)
    {
        double jn = reggeons[i].getJ(), djndt = reggeons[i].getdJdt();

      subroutine dqk21(f,params,a,b,result,abserr,resabs,resasc)
...
c
     ...
     *  resg,resk,reskh,result,uflow,wg,wgk,xgk, params
      integer j,jtw,jtwm1
      external f
c
...
c
      ...
      do 10 j=1,5
        jtw = 2*j
        absc = hlgth*xgk(jtw)
        fval1 = f(centr-absc, params)
        fval2 = f(centr+absc, params)
        fv1(jtw) = fval1
        fv2(jtw) = fval2
        fsum = fval1+fval2
        resg = resg+wg(j)*fsum

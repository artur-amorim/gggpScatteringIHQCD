...
C
     ...
     2                   FSUB, DFSUB, GSUB, DGSUB, GUESS, PARS)
C
C
C**********************************************************************
C
...
C
C                       subroutine dfsub (x , z , df, pars)
C
...
C
C                       subroutine dgsub (i , z , dg, pars)
C
...
C
  ...
  230 CONTINUE
     ...
     5     NFXPNT,FIXPNT,IFLAG,FSUB,DFSUB,GSUB,DGSUB,GUESS, PARS )
C
C...  prepare output
C
      ISPACE(1) = N
...
C----------------------------------------------------------------------
  ...
  360 FORMAT(/53H INSUFFICIENT SPACE TO DOUBLE MESH FOR ERROR ESTIMATE)
     ...
     3           FSUB, DFSUB, GSUB, DGSUB, GUESS, PARS )
C
C**********************************************************************
C
C   purpose
...
C
   60      IF ( IPRINT .LT. 0 )  WRITE (IOUT,510) ITER, RNORM
     ...
     2          3+IFREEZ, FSUB, DFSUB, GSUB, DGSUB, GUESS, PARS )
C
C...       check for a singular matrix
C
...
C
     ...
     2           MODE, FSUB, DFSUB, GSUB, DGSUB, GUESS, PARS )
C*********************************************************************
C
C   purpose
...
C
  110      CALL GSUB (IZETA, ZVAL, GVAL, PARS)
           RHS(NDMZ+IZETA) = -GVAL
           RNORM = RNORM + GVAL**2
...
C
  120      CALL GDERIV (G(IG), NROW, IZETA, ZVAL, DGZ, 1, DGSUB, PARS)
  130      IZETA = IZETA + 1
           GO TO 100
C
...
C
  170        CALL FSUB (XCOL, ZVAL, F, PARS)
             DO 180 JJ = 1, NCOMP
...
C...         the linear case
C
  200        CALL FSUB (XCOL, ZVAL, RHS(IRHS), PARS)
             IRHS = IRHS + NCOMP
C
...
C...       find  rhs  boundary value.
C
  250      CALL GSUB (IZETA, ZVAL, GVAL, PARS)
           RHS(NDMZ+IZETA) = - GVAL
           RNORM = RNORM + GVAL**2
...
C
      RETURN
      END
      SUBROUTINE GDERIV ( GI, NROW, IROW, ZVAL, DGZ, MODE, DGSUB, PARS)
C
C**********************************************************************
C
C   purpose:
...
C
   ...
   60 GI(IROW,MSTAR+J) = DG(J)
     ...
     1                   DF, ACOL, DMZO, NCOMP, DFSUB, MSING, PARS)
C
C**********************************************************************
C
C   purpose:
